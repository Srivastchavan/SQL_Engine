 ...
 * @version 1.1
 *          <p>
 *          This code is (c) 2018 Chris Irwin Davis
 *
 */
public class HexDump {
	...
	/**
	 * <p>
	 * This method will display the byte contents of the RandomAccessFile raf to
	 * Standard Out (stdout)
	 */
	static void displayHexDump() {
		...
		try {
			...
			/*
			 * This int is used to keep track of the current pointer location. This is more
			 * efficient than re-executing raf.getFilePointer() every byte. But it is less
			 * safe since it is possible for thisByteOffset to become out of sync with
			 * raf.getFilePointer() if the value is modified other than a single increment
			 * for each iteration of the byte-by-byte while loop.
			 */
			int thisByteOffset = 0;

			/*
			 * Collect the length of the file in bytes so we'll know when to exit the while
			 * loop
			 */
			...
			/*
			 * Using the while condition below instead of the line above is potentially
			 * unsafe if the code is modified!!!
			 */
			...
			/*
			 * This inserts a newline at the end of the hex dump so that the command prompt
			 * will be at the start of the next line.
			 */
			out.println();
		} /* End of TRY block */
		catch (IOException e) {
